...
multiple:
		docker buildx build --tag sineverba/testphp74xc --platform linux/amd64,linux/arm/v6,linux/arm/v7 --push .
...
test:
		...
		@docker run --rm sineverba/testphp74xc php -r "xdebug_info();" | grep "3.0.2"
